...
import sys
sys.path.append("./vilbert/datasets")
from talk2car import Talk2Car
T2C_ROOT = "/cw/liir/NoCsBack/testliir/thierry/3rd_place/data"
...
class FeatureExtractor:
    ...
    def get_batch_proposals(self, images, im_scales, im_infos, commands):
        ...
        for idx, img_info in enumerate(im_infos):
            boxes_tensor = torch.tensor(commands[idx]["referred_object"])
            boxes_tensor[2] += boxes_tensor[0]
            boxes_tensor[3] += boxes_tensor[1]
            boxes_tensor = boxes_tensor.unsqueeze(0).to("cuda")
            #self.t2c.get_raw_boxes_for_sample(commands[idx]).to("cuda")
    ...
    def _save_feature(self, file_name, feature, info):
        file_base_name = "gt_"+str(file_name).split(".")[0]
...
if __name__ == "__main__":
    ...
    print("Number of params", count_parameters(feature_extractor.detection_model))
    exit()
