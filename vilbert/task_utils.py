...
def ForwardModelsVal(args, task_cfg, device, task_id, batch, model, task_losses):
    ...
    batch = tuple(t.cuda(device=device, non_blocking=True) if type(t) is not list else t for t in batch)
    ...
    elif task_id in ["TASK19", "TASK20"]:
        features, spatials, image_mask, question, target, input_mask, \
        segment_ids, co_attention_mask, question_id, command_token,\
        boxes = (
            batch
        )
    else:
    ...
    if int(task_id[4:]) == 20:
        task_tokens = question.new().resize_(question.size(0), 1).fill_(19)
    else:
...
def LoadDatasets(args, task_cfg, ids, split="trainval"):
    ...
    for features_h5path in task_feature_reader1.keys():
        if features_h5path != "":
            if "feats_dim" in task_cfg[task]:
                task_feature_reader1[features_h5path] = ImageFeaturesH5Reader(
                    features_h5path, args.in_memory, task_cfg[task]["feats_dim"]
                )
            else:
                task_feature_reader1[features_h5path] = ImageFeaturesH5Reader(
...
def EvaluatingModel(
    ...
    if task_cfg[task_id]["type"] == "VL-classifier":
        ...
        for i in range(select_idx.size(0)):
            ix = select_idx[i].item() - 1 # -1 because the first logit is for the full image
            results.append(
                {
                    "id": question_id[i],
                    "target": ix,
                    "IOU": select_target[i].item(),
                    "command_token": command_token[i],
                    "box": torch.tensor(boxes[ix])
                }
            )
