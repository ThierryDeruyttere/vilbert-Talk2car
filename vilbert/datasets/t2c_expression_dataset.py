...
class T2C_ReferExpressionDataset(Dataset):
    def __init__(
        self,
        task: str,
        dataroot: str,
        ...
        self.t2c = Talk2Car(root=dataroot, split=self.split,
                            vocabulary=os.path.join(dataroot, "vocabulary.txt"),
    ...
    def _load_annotations(self, clean_datasets):
        ...
        for ix, cmd in enumerate(self.t2c.data.values()):
            #image_id = ix
            ref_id = ix
            sent_id = ix
            refBox = cmd["referred_object"] if self.split != "test" else None
            caption = cmd["command"]
            entries.append(
                {
                    "caption": caption,
                    ...
                    "boxes": [x["bbox"] for x in cmd["centernet"]],
                    #"img": cmd["img"]
                }
            )
    ...
    def __getitem__(self, index):
        ...
        if self.split == "train":
            gt_features, gt_num_boxes, gt_boxes, gt_boxes_ori = self._gt_image_features_reader["gt_{}".format(image_id)]
        ...
        while len(image_mask) < self.max_region_num:
            image_mask.append(0)
        #if self.split == "test":
        image_mask[0] = 0 # because the first one is the image
        ...
        command_token = entry["command_token"]
