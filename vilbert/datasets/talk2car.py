...
class Talk2Car(data.Dataset):
    def __init__(self, root, split,
                 ...
                 transform=None, num_rpns_per_image=16):
        self.root = root
        self.split = split
        ...
        self.num_rpns_per_image = num_rpns_per_image  # We only use 16 RPN per image
    ...
    def get_raw_boxes_for_sample(self, sample):
        if 'centernet' in sample.keys():
            # First get all the valid bbox (Remove boxes that are to small)
            bbox = torch.stack([torch.LongTensor(sample['centernet'][i]['bbox']) for i in
                                range(self.num_rpns_per_image)])  # num_rpns x 4
            ...
            raise Exception("no centernet boxes")
    ...
    def __getitem__(self, idx):
        ...
        if 'centernet' in sample.keys():
            ...
            for i in range(self.num_rpns_per_image):
                ...
                if valid:
                    rpn_image[i].copy_(
                        torch.nn.functional.interpolate(img[:, rpn_[1]:rpn_[3], rpn_[0]:rpn_[2]].unsqueeze(0),
                                                        (224, 224)).squeeze())
                else:
