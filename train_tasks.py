...
def main():
    ...
    if args.tasks == "20":
        # Do net surgery to cope with our embeddings
        model.bert.v_embeddings.image_embeddings = nn.Linear(task_cfg["TASK"+args.tasks]["feats_dim"], 1024).to(device)
        ...
        for step in range(median_num_iter):
            ...
            for task_id in task_ids:
                if (iterId != 0 and iterId % task_num_iters[task_id] == 0) or (
                    ...
                    isBest = False
                    if task_id not in best_val_score:
                        best_val_score[task_id] = val_score
                        isBest = True
                    else:
                        if best_val_score[task_id] < val_score:
                            isBest = True
                            best_val_score[task_id] = val_score

                    # Save model!
                    if isBest:
                        logger.info("** ** * Saving new best model on validation ** ** * ")
                        model_to_save = (
                            model.module if hasattr(model, "module") else model
                        )  # Only save the model it-self
                        output_model_file = os.path.join(
                            savePath, "pytorch_best_val.bin"
                        )
...
def evaluate(
    ...
    model.eval()
    print(len(task_dataloader_val[task_id]))
    ...
    logger_score = tbLogger.showLossVal(task_id, task_stop_controller)
    model.train()
    return score
